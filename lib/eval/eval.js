...
const { parse, stringify, unit, computer, stripCalc } = require('../utils');
...
// const add = computer('+');
// const subtract = computer('-');
// const divide = computer('/');
// const multiply = computer('*');
...
const nameTransformer = (options = {}) => (ast) => {
  csstree.walk(ast, {
...
const operatorTransformer = (options = {}) => (ast) => {
  const valueTypes = ['Number', 'Dimension', 'Function', 'Parentheses', 'String', 'Percentage'];
...
const functions = {
  ...
  _var,
  ...Object.fromEntries(
    ['hsl', 'hsla', 'rgb', 'rgba'].map(
      (name) => ([name, function (...args) {
...
function evaluate(input, context = {}) {
  ...
  // input = String(input).trim().replace(/^[(]+(.*)[)]+$/, '$1');
  ...
  input = String(input);
  input = stringify((transformers || []).reduce((ast, transformer) => transformer.call(this, ast), ast));
