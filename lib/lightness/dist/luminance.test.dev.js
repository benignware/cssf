...
describe('luminance', function () {
  it('retrieves luminance from hsl', function () {
    var hsl = 'hsl(123, 43%, 60%)';
    var s = luminance(hsl);
    assert.strictEqual(evaluate(s), 0.6);
  });
  it('retrieves luminance from hex', function () {
    var hsl = 'hsl(123, 43%, 60%)';
    var hex = chroma(hsl).hex();
    var s = luminance(hex);
    assert.strictEqual(evaluate(s), chroma(hsl).hsl()[2]);
  });
  it('retrieves luminance from from rgb', function () {
    var hsl = 'hsl(123, 43%, 60%)';
    var rgb = "rgb(".concat(chroma(hsl).rgb().join(', '), ")");
    var s = luminance(rgb);
    assert.strictEqual(evaluate(s), chroma(hsl).hsl()[2]);
  });
  it('retrieves luminance from rgb dynamically', function () {
