...
describe('Color Conversions', () => {
  ...
  it('should convert rgb to hwb for color red', () => {
    expect(rgbToHwb(255, 0, 0).map(c => e(`calc(${c})`))).to.deepCloseTo([0, 0, 0], 0.5);
  });

  it('should convert hwb to rgb for color red', () => {
    expect(hwbToRgb(0, 0, 0).map(c => e(`calc(${c})`))).to.deepCloseTo([255, 0, 0], 0.5);
  });


  it('should convert rgb to hwb for color blue', () => {
    expect(rgbToHwb(0, 0, 255).map(c => e(`calc(${c})`))).to.deepCloseTo([240, 0, 0], 0.5);
  });

  it('should convert hwb to rgb for color blue', () => {
    expect(hwbToRgb(240, 0, 0).map(c => e(`calc(${c})`))).to.deepCloseTo([0, 0, 255], 0.5);
  });

  it('should convert rgb to hwb for color green', () => {
    expect(rgbToHwb(0, 255, 0).map(c => e(`calc(${c})`))).to.deepCloseTo([120, 0, 0], 0.5);
  });
  ...
  it('should convert rgb to hwb for color black', () => {
    expect(rgbToHwb(0, 0, 0).map(c => e(`calc(${c})`))).to.deepCloseTo([0, 0, 1], 0.5);
  });

  it('should convert hwb to rgb for color black', () => {
    expect(hwbToRgb(0, 0, 1).map(c => e(`calc(${c})`))).to.deepCloseTo([0, 0, 0], 0.5);
  });

  // it('should convert rgb to hwb for color white', () => {
  //   expect(rgbToHwb(255, 255, 255).map(c => e(`calc(${c})`))).to.deepCloseTo([0, 0, 0], 0.5);
  // });

  // it('should convert hwb to rgb for color white', () => {
  //   expect(hwbToRgb(0, 0, 0).map(c => e(`calc(${c})`))).to.deepCloseTo([255, 255, 255], 0.5);
  // });

  // it('should convert rgb to hwb for color gray', () => {
  //   expect(rgbToHwb(128, 128, 128).map(c => e(`calc(${c})`))).to.deepCloseTo([0, 0, 0.5], 0.5);
  // });

  // it('should convert hwb to rgb for color gray', () => {
  //   expect(hwbToRgb(0, 0, 0.5).map(c => e(`calc(${c})`))).to.deepCloseTo([128, 128, 128], 0.5);
  // });
});
